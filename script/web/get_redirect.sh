...
TIMEOUT=10 # Giới hạn thời gian tìm kiếm
...
for DIR in "$NGINX_SITE_DIR" "$NGINX_ENABLED_DIR"; do
  if [ -d "$DIR" ]; then
    echo "Đang kiểm tra trong thư mục: $DIR"
    # Sử dụng timeout để giới hạn thời gian tìm kiếm
    timeout "$TIMEOUT" find "$DIR" -type f -name "*.conf" | while read -r conf_file; do
